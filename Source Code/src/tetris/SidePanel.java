...
public class SidePanel extends JPanel {
	...
	public SidePanel(Tetris tetris) {
		...
		setPreferredSize(new Dimension(280, BoardPanel.PANEL_HEIGHT));
	...
	public void paintComponent(Graphics g) {
		...
		g.drawString("THÔNG TIN", SMALL_INSET, offset = STATS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("Cấp độ: " + tetris.getLevel(), LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Điểm: " + tetris.getScore(), LARGE_INSET, offset += TEXT_STRIDE);

		g.setFont(LARGE_FONT);
		g.drawString("HƯỚNG DẪN", SMALL_INSET, offset = CONTROLS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("Phím mũi tên lên: xoay khối", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Phím mũi tên trái: di chuyển sang trái", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Phím mũi tên phải: di chuyển sang phải", LARGE_INSET, offset += TEXT_STRIDE);
                g.drawString("Phím mũi tên xuống: tăng tốc độ rơi", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Phím Cách: tạm dừng", LARGE_INSET, offset += TEXT_STRIDE);

		g.setFont(LARGE_FONT);
		g.drawString("KẾ TIẾP:", SMALL_INSET, 70);
